...
- **SuperEllipse の軸付近の最近点**: `SuperEllipse{ 0, 0, 100, 100, 8 }` と点 `(-100.001, 0)` の距離は `0.001` だが、`Distance()` / `SignedDistance()` は約 `0.0289` を返す。`n = 4` でも `SignedDistance()` が軸上の境界点で負を返し、境界から外向きの `Raycast()` がヒットを返さない。角度からの境界評価と最近点探索を見直し、`Distance` / `ClosestPoints` と `SignedDistance` / `ClosestPointOnBoundary` の重複処理を揃える。軸上だけでなくその近傍も回帰対象にする。
- **離れた曲線間の精度と処理量**: 円・楕円・SuperEllipse・Bezier を含む距離計算は、固定分割した辺の総当たりと 80 回の局所探索を使い、接触に近い配置で距離を過大評価する。楕円同士で正解 `0.01` に対して約 `0.0193`、二次 Bezier 同士で正解 `0.0001` に対して約 `0.00185` を再現。`Distance()` / `ClosestPoints()` の共通探索が対象。凸形状向けの解法、既存の点・楕円ソルバの再利用、凹 SuperEllipse・Bezier 向けの探索を比較し、精度と実行時間を同時に改善する。分割数・反復数の増加だけで対応しない。
