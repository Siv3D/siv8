...
- **離れた曲線間の精度と処理量**: 円・楕円・SuperEllipse・Bezier を含む距離計算は、固定分割した辺の総当たりと 80 回の局所探索を使い、接触に近い配置で距離を過大評価する。楕円同士で正解 `0.01` に対して約 `0.0193`、二次 Bezier 同士で正解 `0.0001` に対して約 `0.00185` を再現。`Distance()` / `ClosestPoints()` の曲線間探索と、そこで使う SuperEllipse の角度からの境界評価が対象。凸形状向けの解法、既存の点・楕円ソルバの再利用、凹 SuperEllipse・Bezier 向けの探索を比較し、精度と実行時間を同時に改善する。分割数・反復数の増加だけで対応しない。
