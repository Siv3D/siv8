...
- 片軸を負倍率で反転した Polygon では、輪郭の向きが反転しても法線の向きを固定しているため、ヒット法線が内向きになる。Polygon / MultiPolygon の外周・穴について、輪郭の向きに基づく法線生成に揃える。
- 境界の一時配列を生成せずに走査する形を検討する。形状別の内部判定に必要な計算も整理し、通常形状・多数頂点・多数要素で処理時間と確保を比較する。
