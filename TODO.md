...
- **円・円弧と Bezier の距離**: 円は中心との最近点計算を再利用し、円弧は中心と両端点に対する極値候補を利用する方法を検証する。`Circle` / `RoundRect` と Bezier2/3 の分割探索を減らし、交差時の共通点取得も含めて既存の契約と比較する。他の図形へ広げる際は専用処理と速度を比較する。凸 SuperEllipse と円では、現行の専用処理が中心点との距離を経由する方法より速い。
