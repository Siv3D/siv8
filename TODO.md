...
- **Bezier と一般の SuperEllipse の交差**: 非直線 Bezier と `n == 1, 2` 以外の SuperEllipse は、凹形状・凸形状とも、曲線が中心を通過する場合にも見落としがある。正規化した制御点の範囲から除外し、実際の曲線上の点で交差を確認する上限付き探索を比較する。接触付近の未確定時は既定の保守的判定に合わせ、`pow()` の評価回数と丸め誤差を含めて検証する。
