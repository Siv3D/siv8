...
- **Bezier の点・直線、SuperEllipse の菱形との交差**: Bezier 同士と `Bezier × SuperEllipse`（`n != 2`）の折れ線近似で偽陽性・偽陰性を確認。まず点・直線に縮退する Bezier と `n == 1` の菱形を既存の点・線分・四角形判定へ渡す。折り返す直線 Bezier の範囲は端点だけでは決まらないため、極値を含めて求める。形状の正規化を共有し、`IntersectsAt()` / `ClosestPoints()` に粗い曲線近似が残らないかも確認する。
- **一般の Bezier 同士の交差**: 二次・三次の全組合せで誤判定し、同じ曲線の反転や次数上げでも結果が変わる。偽陽性は `Distance() == 0` と曲線上にない最近点を返す原因にもなる。距離ソルバの候補・分割・下限評価の再利用を検討するが、近い曲線では現行の評価上限に達しても交差の有無を確定できない。判定だけに必要な早期終了と、未確定時・丸め誤差内の扱いを決める。
- **Bezier と一般の SuperEllipse の交差**: 凹形状・凸形状とも、曲線が中心を通過する場合にも見落としがある。正規化した制御点の範囲から除外し、実際の曲線上の点で交差を確認する上限付き探索を比較する。接触付近の未確定時は既定の保守的判定に合わせ、`pow()` の評価回数と丸め誤差を含めて検証する。`n == 1` / `n == 2` と縮退は専用処理へ分ける。
- **曲線の交点・共通点取得**: `IntersectsAt()` の固定標本・局所探索は、交差判定が正しくても交点を取りこぼしたり、曲線からずれた点を返したりする。`Intersects()` による早期除外と、`ClosestPoints()` の共通点取得への波及を含めて見直す。
