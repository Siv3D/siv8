...
- **優先: モデリング用の自動プレビューと診断。** 正面・側面・背面・斜めの固定視点、共通のスケールと照明、材質色表示と部品 ID 表示を切り替えられる開発用ツールを整える。画像から問題の PartID を特定できる対応表と、頂点数・三角形数・材質数も出す。意図した接触や間隔の検査は上記の診断候補と連携させる。Siv3D の描画 API を先行確定せず、まず `Test/Manual/` の再現可能な補助ツールとして評価する。
