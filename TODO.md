...
- **既存ソルバへの還元**: 凹 SuperEllipse 同士（菱形を含む）の離れた最近点対は、少なくとも一方が軸上の先端になる。先端と相手の点距離ソルバを使う経路へまとめ、固定分割と 2 変数の局所探索を外す。円相当の Ellipse / SuperEllipse も既存の Circle と同じ距離計算へ還元する。例えば `SuperEllipse{ 0, 0, 1, 1, 0.5 }` と、中心 `(0.75, 0.75)`・半径 `sqrt(0.5) - 1e-8` の円では、円を表す型によって距離誤差と処理量が大きく異なる。
- **凹 SuperEllipse × 一般の凸曲線**: 楕円・凸 SuperEllipse との曲線内部同士の最近点が残る。現行の局所探索は微小な隙間を過大評価し、引数順で結果が変わる。点距離ソルバを使う 1 変数化を比較するが、粗い標本から局所最小だけを追う試作にも取りこぼしがあるため、そのまま採用しない。距離の下界で区間を除外する上限付き探索を検証し、未確定の交差判定から距離ゼロを推定しない。
