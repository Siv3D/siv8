...
- **離れた曲線間の精度と処理量**: 円・楕円同士以外に残る汎用の `Distance()` / `ClosestPoints()` は、固定分割した辺の総当たりと 80 回の局所探索を使い、接触に近い配置で距離を過大評価する。二次 Bezier 同士で正解 `0.0001` に対して約 `0.00185` を再現。SuperEllipse・Bezier、線分・円弧との組合せをまとめて見直し、SuperEllipse の角度からの境界評価も対象とする。凸形状向けの解法、既存の点・楕円ソルバの再利用、凹 SuperEllipse・Bezier 向けの探索を比較し、精度と実行時間を同時に改善する。分割数・反復数の増加だけで対応しない。
