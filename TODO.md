...
- `CorrectOne()` で最大成分だけを三角形化するには、`area()` の計算を描画用 `Float2` 頂点から幾何計算用 `Vec2` 輪郭へ統一するかを先に決める。現在の面積比較には Float2 への丸めが反映されるため、修復後の輪郭面積で先に選択すると返す成分が変わる場合がある。
- 面積計算を変更する場合は、大きな平行移動、微小な穴、ほぼ同面積の成分で `area()`・`CorrectOne()` の整合性を確認する。
