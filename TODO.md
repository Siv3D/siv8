...
- Windows で全自動テストを実行し、ファイル日時の取得・変換失敗、Unicode の相対パス、Downloads の取得を確認する。OEM コードページが UTF-8 以外の環境、夏時間のあるタイムゾーン、Downloads の移動先を使用する環境での確認も行う。
- Linux のビルド・自動テスト手順を整備し、macOS と共通の実装を検証する。

### 基盤の見直し

- 文字列だけで行うパス操作と、存在確認・絶対パス化・リンク解決を伴う操作の契約を整理する。区切り文字、ルート・UNC、末尾の区切り文字、存在しないパス、リソースパスの扱いを実装・テスト・利用箇所と照合する。
- OS ネイティブ形式への変換を整理し、属性取得・絶対パス化・列挙で繰り返している変換や OS 呼び出しを計測する。測定結果から共通処理とプラットフォーム固有処理の分担を決める。
- 空・0・false に集約している失敗情報について、呼び出し側が必要とする区別を調べる。部分結果、コピー・削除の途中失敗、一時ファイル名の生成とファイル確保の責務を明確にする。
- リンクを含む列挙・集計・コピー・削除の一貫性、ファイル日時の意味と精度を確認する。OS ごとの制約と、ライブラリとして統一する挙動を分けて設計する。
- 通常のファイル操作、多数のエントリ、深い階層、長いパスを含む性能測定とテストを整備し、割り当て・OS 呼び出し・走査時間を基準に改善の優先順位を決める。
