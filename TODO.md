...
- **凹 SuperEllipse × 一般の凸曲線**: 円・菱形以外の楕円・凸 SuperEllipse との曲線内部同士の最近点が残る。現行の局所探索は微小な隙間を過大評価し、引数順で結果が変わる。点距離ソルバを使う 1 変数化を比較するが、粗い標本から局所最小だけを追う試作にも取りこぼしがあるため、そのまま採用しない。距離の下界で区間を除外する上限付き探索を検証し、未確定の交差判定から距離ゼロを推定しない。
