...
- **共通点の確認と距離 0 の整合性**: `ClosestPoints()` の一般経路は、`Intersects()` が true でも共通点を確認できない場合に assert し、Release では片方の代表点を共通点として返す。上限付きの交差判定と近似交点取得を前提に、未確定の交差と確認済みの共通点を区別する。曲線の接触・浅い重なり・近接した非交差を対象に、両図形に属する点と距離の整合性を検証する。
