...
## `Siv3D/include/Siv3D/Pattern/`

### 描画変換に追従する模様（Metal → D3D11）

- [採用した座標設計](docs/renderer2d/pattern-coordinates.md)に従い、Metal を完成・検証してから D3D11 へ移植する。理解とレビューの区切りとして以下を別段階で進め、各段階の挙動・差分・検証結果を説明する。
- Metal の座標経路を切り替える。変換前の頂点位置を既存の `float2` varying で渡し、全 Pattern PS がそれを参照する。Pattern のパラメータ転送から RMS 補正を外し、全図形の Pattern 描画経路を統一する。通常のテクスチャ UV 経路と D3D11 の挙動を維持し、基本的な描画テストと公開ドキュメントを伴わせる。
- Metal の検証を完成させる。変換の組み合わせ・viewport・模様の境界連続性・描画状態の切り替え・バッチ境界を確認し、目視サンプルを新仕様に更新する。macOS の全自動テストを通し、カスタムシェーダの varying 契約と性能上の影響を整理する。
- D3D11 に同じ座標経路を移植する。Windows でシェーダのビルドと自動・目視検証を行い、移行期間の RMS 転送経路とプラットフォーム差の説明を整理する。macOS だけで D3D11 の実行確認済みとはしない。
