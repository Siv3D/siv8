...
- [Pattern の座標・パラメータ命名](docs/renderer2d/pattern-payload.md#shader-side-interpretation)、Truchet の seed 復元関数、[色加算の共通化と背景色の PMA 命名](docs/renderer2d/pattern-payload.md#color-composition)、[MSDF の状態・パラメータ命名](docs/renderer2d/msdf-font.md#shader-parameter-interpretation)、[XY 座標変換の共通化](docs/renderer2d/pattern-payload.md#shared-xy-transform)について、macOS で Metal のコンパイル、Pattern・MSDF 関連テスト、全自動テストを実行する。
