...
- **離れた曲線間の精度と処理量**: SuperEllipse と直線辺・円弧、Bezier と点・他種類の図形、凹 SuperEllipse・菱形（`n <= 1`）と曲線、楕円と円弧の組合せでは、汎用の `Distance()` / `ClosestPoints()` に固定分割と 80 回の局所探索が残る。接触付近の距離誤差と処理量をまとめて検証し、SuperEllipse の角度からの境界評価も見直す。既存の点・楕円ソルバの再利用、直線辺の厳密な処理、凹曲線向けの探索を比較し、分割数・反復数の増加だけで対応しない。
