...
- 重なる図形の `ClosestPoints()` が共通点を得るために作る交点配列・代表点配列と、`IntersectsAt()` の結果整理用配列の確保を削減する。共通点が 1 点あればよい経路の早期終了、代表点の直接走査、交点配列の領域再利用をまとめて検討する。返却する交点配列の確保と、削減可能な作業用の確保を区別する。
