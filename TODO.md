...
- **優先: 取り付け座標系を返すレシピの拡充。** `Mesh3DPlacement::Align()` と形状なし Part を使い、形状と取り付け座標系を同じ寸法入力から生成するレシピを評価する。部品の寸法を変更しても接続を保てることを評価基準とする。レシピ間で座標系を受け渡す標準化が必要かを確認し、専用の AnchorID や文字列検索は不足が見つかってから検討する。`setMesh()` による自動追従や常時評価する制約機構は前提にしない。
- **優先: モデリング用の診断。** `Test/Manual/Mesh3DPreview.md` の固定視点・材質色プレビューに、部品 ID 表示と画像から問題の PartID を特定できる対応表を追加する。材質数の集計、細部を指定する接写、意図した接触や間隔の検査も評価する。Siv3D の描画 API を先行確定せず、開発用の補助ツールとして設計する。
