...
TEST_CASE("Test::OutputPath stays anchored when the working directory changes")
{
	...
	CHECK((Test::OutputPath(U"environment/日本語.bin")) == (expected));
	...
	CHECK((Test::OutputPath(U"environment/日本語.bin")) == (expected));
	{
		...
		REQUIRE(writer);
		REQUIRE((writer.write("test", 4)) == (int64{ 4 }));
	}
	CHECK((Blob{ expected }) == (Blob("test", 4)));
}

TEST_CASE("Test::OutputPath accepts child files and directories")
{
	const FilePath root = FileSystem::FullPath(U"../../Test/output/");
	CHECK((Test::OutputPath(U".hidden")) == ((root + U".hidden")));
	CHECK((Test::OutputPath(U"nested/directory/")) == ((root + U"nested/directory/")));
	CHECK((Test::OutputPath(U"name..txt")) == ((root + U"name..txt")));
}
