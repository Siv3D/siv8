...
namespace
{
	...
	struct TrackedArrayValue
	{
		...
		~TrackedArrayValue() { CHECK((state->live.erase(this)) == (size_t{ 1 })); }
	...
	{
		REQUIRE((source.size()) == (size_t{ 8 }));
		for (size_t i = 0; i < source.size(); ++i) { CHECK((source[i].value()) == (static_cast<int>(i))); }
	}
}
	...
	for (int operation = 0; operation < 4; ++operation)
	{
		for (int failAt = 0; failAt <= 8; ++failAt)
		{
			...
			ArrayFaultState state;
			{
				...
				CHECK((failed) == (failAt < 8));
				CheckOriginal(source);
				CHECK((state.live.size()) == (source.size()));
				CHECK((state.blocks.size()) == (blocks));
			}
...
TEST_CASE("Array.exception.reserve_relocation")
{
	for (int failAt = 0; failAt < 8; ++failAt)
	{
		...
		ArrayFaultState state;
		{
			...
			CHECK((source.capacity()) == (capacity));
			CHECK((state.live.size()) == (source.size()));
			CHECK((state.blocks.size()) == (blocks));
			state.copies.arm(-1);
	...
	for (int failAt = 0; failAt <= 4; ++failAt)
	{
		...
		ArrayFaultState state;
		{
			...
			try
			{
				...
				REQUIRE((result.size()) == (size_t{ 4 }));
				for (size_t i = 0; i < result.size(); ++i) { CHECK((result[i].value()) == (static_cast<int>(i * 2 + 1))); }
			}
			catch (const InjectedArrayFailure&) { failed = true; }
			CHECK((failed) == (failAt < 4));
	...
	for (int failAt = 0; failAt <= 6; ++failAt)
	{
		...
		ArrayFaultState state;
		{
			...
			try
			{
				...
				for (size_t i = 0; i < result.size(); ++i) { CHECK((result[i].value()) == (static_cast<int>(i))); }
			}
			catch (const InjectedArrayFailure&) { failed = true; }
			CHECK((failed) == (failAt < 6));
			CHECK((state.live.size()) == (size_t{ 6 }));
...
namespace
{
	...
	void CheckArrayAllocatorPropagation()
	{
		...
		ArrayFaultState first, second;
		{
			...
			destination = source;
			CHECK((destination.get_allocator().state) == (Propagate ? &first : &second));
		...
		CHECK(first.blocks.empty()); CHECK(second.blocks.empty());
		{
			...
			CHECK((destination.get_allocator().state) == (Propagate ? &first : &second));
			CHECK((destination.size()) == (size_t{ 128 }));
			CHECK(destination.all([](T v) { return v == T{ 1 }; }));
		...
		CHECK(first.blocks.empty()); CHECK(second.blocks.empty());
		{
			...
			a.swap(b);
			CHECK((a.size()) == (size_t{ 64 })); CHECK((b.size()) == (size_t{ 128 }));
			CHECK((a.get_allocator().state) == (Propagate ? &second : &first));
			CHECK((b.get_allocator().state) == (&first));
...
TEST_CASE("Array.exception.bool_storage_transition")
{
	...
	ArrayFaultState state;
	{
		...
		CHECK((flags.data()) == (storage));
		CHECK((flags.size()) == (smallCapacity));
		CHECK(flags.all());
		CHECK((state.blocks.size()) == (blocks));
		state.allocations.arm(-1);
		flags.push_back(false);
		CHECK((flags.size()) == (smallCapacity + 1));
		CHECK_FALSE(flags.back());
		CHECK((flags.get_allocator().state) == (&state));
		flags.resize(1, false);
		flags.shrink_to_fit(); // Shrinking is non-binding; only contents and ownership are promised.
		CHECK((flags.size()) == (size_t{ 1 }));
		flags.release();
		CHECK(flags.empty());
		CHECK((state.blocks.size()) == (emptyBlocks));
