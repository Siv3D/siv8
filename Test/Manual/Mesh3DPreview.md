...
- Self-tests cover all six axis directions, depth ordering independent of draw
  ...
  reconstruction of Siv3D base colors at metallic values 0, 0.5, and 1.
...
def render(model, azimuth, elevation, size=640, unlit=False):
...
def main():
    ...
    model = read_obj(args.obj)
    args.output.mkdir(parents=True, exist_ok=True)
    views = [('front', 180, 0), ('back', 0, 0), ('left', -90, 0), ('right', 90, 0), ('top', 0, 90), ('iso', 140, 25)]
    for name, az, el in views:
        Image.fromarray(render(model, az, el, args.size, args.unlit)).save(args.output / f'{args.obj.stem}_{name}.png')
    print(f'{len(model[0])} vertices, {len(model[1])} triangles; wrote {len(views)} views to {args.output}')
