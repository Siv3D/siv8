...
namespace
{
	...
	void CheckSnapshot(const Assembly::BakedMesh& actual, const Assembly::BakedMesh& expected)
	{
		...
		REQUIRE((actual.parts.size()) == (expected.parts.size()));
		REQUIRE((actual.materials.size()) == (expected.materials.size()));
		for (size_t i = 0; i < actual.parts.size(); ++i)
		{
			...
			CHECK((a.range.vertexOffset) == (e.range.vertexOffset));
			CHECK((a.range.vertexCount) == (e.range.vertexCount));
			CHECK((a.range.triangleOffset) == (e.range.triangleOffset));
			CHECK((a.range.triangleCount) == (e.range.triangleCount));
			CHECK(a.worldTransform == e.worldTransform);
		}
		for (size_t i = 0; i < actual.materials.size(); ++i)
		{
			CHECK((actual.materials[i].name) == (expected.materials[i].name));
			CHECK((actual.materials[i].baseColor) == (expected.materials[i].baseColor));
		}
	}
}
...
TEST_CASE("Mesh3DAssembly::empty and transform-only parts")
{
	Assembly a;
	a.reserve(2, 2, 4);
	CHECK((a.meshCount()) == (size_t{ 0 }));
	CHECK((a.materialCount()) == (size_t{ 0 }));
	CHECK((a.partCount()) == (size_t{ 0 }));
	...
	REQUIRE((output.parts.size()) == (size_t{ 2 }));
	CHECK(output.parts[0].range.isEmpty());
	CHECK(output.parts[1].range.isEmpty());
	CHECK((output.parts[1].worldTransform.transformPoint(Float3::Zero())) == ((Float3{ 5, 7, 9 })));
	CHECK((a.computeWorldTransform(child).value().transformPoint(Float3::Zero())) == ((Float3{ 5, 7, 9 })));
	a.clear();
	CHECK((a.partCount()) == (size_t{ 0 }));
...
TEST_CASE("Mesh3DAssembly::mesh ownership and registration failure")
{
	...
	CHECK((a.getMesh(id)->vertices.data()) == (storage));
	CHECK((a.meshCount()) == (size_t{ 1 }));
	const auto copyID = a.addMesh(*a.getMesh(id)).value();
	CHECK((a.getMesh(copyID)->vertices.data()) != (storage));
	CHECK((a.addMesh(Mesh3D{}).error().code) == (Mesh3DErrorCode::InvalidGeometry));
	Mesh3D invalid = Mesh3D::Box();
	invalid.indices[0].i0 = static_cast<uint32>(invalid.vertexCount());
	CHECK((a.addMesh(invalid).error().code) == (Mesh3DErrorCode::InvalidGeometry));
	CHECK((a.setMesh(id, invalid).error().code) == (Mesh3DErrorCode::InvalidGeometry));
	CHECK((a.setMesh(MissingMesh, Mesh3D::Box()).error().code) == (Mesh3DErrorCode::InvalidArgument));
	CHECK((a.meshCount()) == (size_t{ 2 }));
...
TEST_CASE("Mesh3DAssembly::part references and failed edits")
{
	...
	{
		CHECK((a.addPart(part).error().code) == (Mesh3DErrorCode::InvalidArgument));
		CHECK((a.setPart(child, part).error().code) == (Mesh3DErrorCode::InvalidArgument));
	}
	CHECK((a.setPart(root, { .parent = root }).error().code) == (Mesh3DErrorCode::InvalidArgument));
	CHECK((a.setPart(root, { .parent = child }).error().code) == (Mesh3DErrorCode::InvalidArgument));
	CHECK((a.setPart(child, { .parent = child }).error().code) == (Mesh3DErrorCode::InvalidArgument));
	CHECK((a.setPart(MissingPart, {}).error().code) == (Mesh3DErrorCode::InvalidArgument));
	CHECK((a.setPlacement(MissingPart, Vec3::Zero()).error().code) == (Mesh3DErrorCode::InvalidArgument));
	CHECK((a.setMaterial(MissingMaterial, Material{}).error().code) == (Mesh3DErrorCode::InvalidArgument));
	CHECK((a.computeWorldTransform(MissingPart).error().code) == (Mesh3DErrorCode::InvalidArgument));
	...
	CHECK((a.getPart(child)->name) == (U"renamed"));
	...
	CHECK((a.computeWorldTransform(reparented).value().transformPoint(Float3::Zero())) == ((Float3{ 2, 0, 0 })));
}
...
TEST_CASE("Mesh3DAssembly::hierarchy transforms and mirrored geometry")
{
	...
	CHECK((a.getMesh(shape)->vertices[0].pos) == (mesh.vertices[0].pos));
...
TEST_CASE("Mesh3DAssembly::reusable bake and size budgets")
{
	...
	{
		CHECK((a.bake(output, options).error().code) == (Mesh3DErrorCode::SizeLimit));
		CHECK((a.bake(options).error().code) == (Mesh3DErrorCode::SizeLimit));
		CheckSnapshot(output, before);
	}
	REQUIRE(a.setPlacement(part, Vec3{ 2, 3, 4 }));
	REQUIRE(a.bake(output));
	CHECK((output.mesh.vertices.data()) == (vertices));
	CHECK((output.mesh.indices.data()) == (indices));
	CHECK((output.parts.data()) == (parts));
	CHECK((output.materials.data()) == (materials));
...
TEST_CASE("Mesh3DAssembly::stable IDs and independent snapshots")
{
	...
	CHECK((after.parts[1].range.vertexOffset) == (a.getMesh(shape)->vertexCount()));
	CHECK((before.parts[1].range.vertexOffset) != (after.parts[1].range.vertexOffset));
	CHECK((before.materials[0].name) == (U"old"));
	CHECK((after.materials[0].name) == (U"new"));
	...
	CHECK((after.materials[0].baseColor) == ((ColorF{ 1, 0, 0 })));
}
...
TEST_CASE("Mesh3DAssembly::deep hierarchy without recursion")
{
	...
	const auto baked = a.bake().value();
	CHECK((baked.parts.back().worldTransform.transformPoint(Float3::Zero())) == ((Float3{ 4096, 0, 0 })));
	CHECK((a.computeWorldTransform(*parent).value().transformPoint(Float3::Zero())) == ((Float3{ 4096, 0, 0 })));
}
...
TEST_CASE("Mesh3DAssembly::cart wheel rotates about its own center")
{
	...
	for (size_t i = 0; i < after.parts.size(); ++i)
	{
		if (after.parts[i].id == wheel)
		{
			...
			CHECK((after.parts[i].worldTransform.transformPoint(Float3::Zero())) == (before.parts[i].worldTransform.transformPoint(Float3::Zero())));
		}
		else
		{
...
TEST_CASE("Mesh3DAssembly::chest hinge and material editing")
{
	...
	CHECK((chest.assembly.meshCount()) == (size_t{ 4 }));
	CHECK((chest.assembly.partCount()) == (size_t{ 6 }));
	...
	CHECK((hinge) == (closed.parts[1].worldTransform.transformPoint(Float3::Zero())));
	...
	CHECK((gold.materials[0].name) == (U"wood"));
	CHECK((gold.materials[1].name) == (U"gold"));
...
TEST_CASE("Mesh3DAssembly::cart shared wheels and recipe parameters")
{
	for (const uint32 axleCount : { 1u, 2u, 3u })
	{
		...
		CHECK((cart.assembly.meshCount()) == (size_t{ 3 }));
		CHECK((cart.wheels.size()) == ((axleCount * 2)));
		CHECK((cart.assembly.partCount()) == ((1 + axleCount * 3)));
		...
		for (size_t i = 0; i < cart.wheels.size(); ++i)
		{
			...
			CHECK(center.x == Test::Approx((i % 2) ? 1.0 : -1.0));
			CHECK(center.y == Test::Approx(0.5));
		}
		REQUIRE(cart.assembly.setMesh(cart.wheelMesh, Mesh3D::Cylinder(0.5, 0.22, 32)));
		const auto refined = cart.assembly.bake().value();
		CHECK((refined.mesh.vertexCount()) > (baked.mesh.vertexCount()));
