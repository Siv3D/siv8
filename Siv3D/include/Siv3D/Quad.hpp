...
namespace s3d
{
	...
	struct Quad
	{
		...
		/// @brief 座標をオフセットした新しい四角形を返します。
		/// @param x X 軸方向のオフセット量
		/// @param y Y 軸方向のオフセット量
		/// @return オフセットした新しい四角形
		[[nodiscard]]
		constexpr Quad withOffset(value_type x, value_type y) const noexcept;

		/// @brief 座標をオフセットした新しい四角形を返します。
		/// @param v オフセット量
		/// @return オフセットした新しい四角形
		[[nodiscard]]
		constexpr Quad withOffset(position_type v) const noexcept;
		...
		/// @brief 四角形を丸く太らせて作成した、新しい多角形を返します。分割数は半径に応じて自動的に決定されます。
