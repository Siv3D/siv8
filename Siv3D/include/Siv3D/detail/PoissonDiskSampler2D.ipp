...
namespace s3d
{
		...
		, m_grid((static_cast<int32>(m_size.x / m_cellSize) + 1), (static_cast<int32>(m_size.y / m_cellSize) + 1))
	{
		...
		if (not m_grid.indexInBounds(indexY, indexX))
		{
			return;
		}
	...
	inline bool PoissonDiskSampler2D::isComplete() const noexcept
	{
	...
	inline size_t PoissonDiskSampler2D::update(const size_t iterations)
	{
		return update(GetDefaultRNG(), iterations);
	}

	inline size_t PoissonDiskSampler2D::update(Concept::UniformRandomBitGenerator auto&& urbg, const size_t iterations)
	{
		if (isComplete())
		{
			return 0;
		...
		for (size_t i = 0; i < iterations; ++i)
		{
			...
			if (not found)
			{
				m_activeList[randomIndex] = m_activeList.back();
