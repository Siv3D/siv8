...
namespace s3d
{
	...
	struct Bezier3
	{
		...
		/// @brief 三次ベジェ曲線を二次ベジェ曲線に近似変換します（次数低減）。
		/// @return 二次ベジェ曲線（近似）
		[[nodiscard]]
		Bezier2 toBezier2() const noexcept;
...
struct fmt::formatter<s3d::Bezier3>
{
	...
	format_context::iterator format(const s3d::Bezier3& value, format_context& ctx);
};
...
struct fmt::formatter<s3d::Bezier3, s3d::char32>
{
	...
	s3d::BufferContext::iterator format(const s3d::Bezier3& value, s3d::BufferContext& ctx);
};
