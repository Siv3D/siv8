...
namespace s3d
{
	...
	struct Triangle
	{
		...
		/// @see @ref geometry2d_queries
		template <class Shape2DType>
			requires detail::SupportsOverlaps<Triangle, Shape2DType>
		[[nodiscard]]
		...
		/// @see @ref geometry2d_queries
		template <class Shape2DType>
			requires detail::SupportsContains<Triangle, Shape2DType>
		[[nodiscard]]
		...
		/// @brief 3 つの座標から三角形を作成します。
		/// @param p0 1 つ目の頂点の座標
		/// @param p1 2 つ目の頂点の座標
		/// @param p2 3 つ目の頂点の座標
		/// @return 作成された三角形
		[[nodiscard]]
		static constexpr Triangle FromPoints(const position_type& p0, const position_type& p1, const position_type& p2) noexcept;
