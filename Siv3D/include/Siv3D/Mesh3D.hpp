...
namespace s3d
{
	...
	struct Mesh3D
	{
		...
		/// @remark 変換の線形部分が非特異である場合、法線および接線も変換後の接空間と整合するよう変換されます。
		/// @remark 変換の線形部分が特異である場合、法線および接線は変更されません。
		/// @remark UV 座標および三角形インデックスは変更されません。
		/// @remark 変換の線形部分の行列式が負の場合でも三角形の巻き順は変更されません。必要に応じて `reverseWinding()` を使用してください。
		[[nodiscard]]
		...
		/// @return 拡大・縮小した 3D メッシュ
		/// @remark `scale` のすべての成分が非ゼロの場合、法線および接線も変換後の接空間と整合するよう変換されます。
		/// @remark `scale` のいずれかの成分が 0 の場合、法線および接線は変更されません。
		/// @remark UV 座標および三角形インデックスは変更されません。
		/// @remark 拡大率の積が負の場合でも三角形の巻き順は変更されません。必要に応じて `reverseWinding()` を使用してください。
		[[nodiscard]]
