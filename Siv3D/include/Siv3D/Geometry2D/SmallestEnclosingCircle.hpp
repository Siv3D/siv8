...
namespace s3d
{
	namespace Geometry2D
	{
		...
		/// @return 点群 points の最小包含円
		[[nodiscard]]
		Circle SmallestEnclosingCircle(Array<Vec2> points, double tolerance, Concept::UniformRandomBitGenerator auto&& urbg);
		...
		/// @param tolerance 点が円に含まれているかの判定時の許容誤差。相対誤差または絶対誤差がこの値以下であれば、点が円に含まれているとみなします。
		[[nodiscard]]
		Circle SmallestEnclosingCircle(Array<Vec2> points, Concept::UniformRandomBitGenerator auto&& urbg, double tolerance = 1e-8);
	}
}
