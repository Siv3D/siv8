...
namespace s3d
{
	...
	struct OffsetCircularBase
	{
		...
		/// @return 新しい円座標
		[[nodiscard]]
		constexpr OffsetCircularBase withOffsetR(value_type _r) const noexcept;
		...
		/// @return 新しい円座標
		[[nodiscard]]
		constexpr OffsetCircularBase withOffsetTheta(value_type _theta) const noexcept;
...
struct fmt::formatter<s3d::OffsetCircularBase<Float, Oclock>, s3d::char32>
{
	...
	s3d::BufferContext::iterator format(const s3d::OffsetCircularBase<Float, Oclock>& value, s3d::BufferContext& ctx) const
	{
		if (tag.empty())
		{
