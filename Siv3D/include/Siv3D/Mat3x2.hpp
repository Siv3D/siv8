...
namespace s3d
{
	...
	struct Mat3x2
	{
		...
		/// @brief 平行移動を除いた座標変換の RMS 拡大率を返します。
		/// @return 単位ベクトルを全方向に一様に向けたときの、変換後の長さの二乗平均平方根。線形部分がゼロの場合は 0
		/// @remark 等方スケール `s` では `abs(s)`、X 方向 4 倍・Y 方向 1 倍では `sqrt(8.5)` になります。
		[[nodiscard]]
		float rmsScaling() const noexcept;
