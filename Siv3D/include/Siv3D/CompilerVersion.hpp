...
# if (SIV3D_COMPILER(MSVC) && (_MSC_VER < 1943))

	# pragma message("Siv3D のビルドには Visual Studio 2022 version 17.13 以降が必要です。Visual Studio をアップデートするか、最新版をダウンロードしてください。")
	# error "Siv3D requires Visual Studio 2022 version 17.13 or later. Please update your Visual Studio installation or download the latest version."

# endif

// C++ 標準ライブラリのバージョン要件チェック
// C++ Standard Library version requirement check
# if (SIV3D_COMPILER(MSVC) && (_MSVC_STL_UPDATE < 202411L))
