...
namespace s3d
{
	...
	///     Mesh3D mesh = builder.obtainMesh(); // ストレージを取り出し、builder を空にする
	/// }
	/// else
	/// {
	...
	public:
		...
		/// @brief 構築した 3D メッシュを取り出し、ビルダーを空にします。
		/// @return 構築した 3D メッシュ
		/// @remark 頂点配列と三角形インデックス配列のストレージを、要素のコピーや新規確保なしで戻り値へ移譲します。確保済みの容量も戻り値へ移り、ビルダーには残りません。
		/// @remark 呼び出し後のビルダーは、新しく構築した空のビルダーと同じ状態になり、引き続き形状を追加できます。取得済みのメッシュはその後のビルダー操作の影響を受けません。
		/// @remark 空のビルダーからは空のメッシュを返します。内容を参照するだけなら getMesh()、容量を保持して内容を消去するなら clear() を使います。
		[[nodiscard]]
		Mesh3D obtainMesh() noexcept;
