...
namespace s3d
{
	/// @brief 三次ベジェ曲線上を距離で進めるためのユーティリティ
	///
	...
	public:
		...
		/// @brief デフォルトコンストラクタ
		[[nodiscard]]
		Bezier3Walker() = default;

		/// @brief 三次ベジェ曲線からウォーカーを作成します。
		/// @param bezier 三次ベジェ曲線
		///
		/// @remark
		/// 内部では導関数 `B'(t) = m_d0 * t^2 + m_d1 * t + m_d2` の係数のみを保持します。
		[[nodiscard]]
		explicit constexpr Bezier3Walker(const Bezier3& bezier) noexcept;
	...
	private:
		...
		/// @brief 導関数の二次係数（B'(t) = m_d0 * t^2 + m_d1 * t + m_d2）
		Vec2 m_d0{ 0, 0 };

		/// @brief 導関数の一次係数（B'(t) = m_d0 * t^2 + m_d1 * t + m_d2）
		Vec2 m_d1{ 0, 0 };

		/// @brief 導関数の定数項（B'(t) = m_d0 * t^2 + m_d1 * t + m_d2）
		Vec2 m_d2{ 0, 0 };
...
# include "detail/Bezier3Walker.ipp"
