...
namespace s3d
{
	namespace
	{
		...
		[[nodiscard]]
		static bool IsFilledRect(const RectF& rect, const double innerThickness, const double outerThickness) noexcept
		{
	...
	size_t RectF::sideIndexAtLength(double length) const
	{
		length = WrapLength(length, perimeter());
