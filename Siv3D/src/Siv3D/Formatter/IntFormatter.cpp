...
namespace s3d
{
	namespace detail
	{
		...
		[[nodiscard]]
		constexpr const char* GetDigits2(const size_t value) noexcept
		{
		...
		constexpr void Copy2(char* dst, const char* src) noexcept
		{
		...
		constexpr FormatDecimalResult FormatDecimal(char* out, uint64 value, const size_t size) noexcept
		{
			...
			{
				out -= 2;
				Copy2(out, GetDigits2(static_cast<size_t>(value % 100)));
				value /= 100;
			}
			...
			out -= 2;
			Copy2(out, GetDigits2(static_cast<size_t>(value)));
			return{ out, end };
		}
