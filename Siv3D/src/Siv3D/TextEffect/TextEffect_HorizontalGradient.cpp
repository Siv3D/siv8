...
namespace s3d
{
	...
	{
		void HorizontalGradient::draw(const TextureRegion& textureRegion, const GlyphContext& glyphContext) const
		{
			...
			const ColorF c0 = (glyphContext.isColorGlyph ? ColorF{ 1.0, Math::Lerp(m_startColor.a, m_endColor.a, t0) } : m_startColor.lerp(m_endColor, t0));
			const ColorF c1 = (glyphContext.isColorGlyph ? ColorF{ 1.0, Math::Lerp(m_startColor.a, m_endColor.a, t1) } : m_startColor.lerp(m_endColor, t1));
