...
namespace s3d
{
	namespace
	{
		...
		{
			return (System::GetUserInfo().defaultLanguage.lowercased().contains(U"ja") ? 1 : 0);
		}

		static void Show_impl(const bool isError, const int32 number, const int32 messageArrayIndex, const StringView hint)
		{
			const size_t languageIndex = GetLanguageIndex();
			const StringView language = Languages[languageIndex];

			const String messageBody = (Fmt((isError ? Errors : Warnings)[messageArrayIndex][languageIndex])(hint));
			const StringView messageFooter = AdditionalMessages[languageIndex];
	...
	namespace Troubleshooting
	{
		...
		{
			Show_impl(false, FromEnum(id), (FromEnum(id) - FromEnum(Warning::InefficientAssetCreation)), hint);
		}

		void Show(const Error id, const StringView hint)
		{
			Show_impl(true, FromEnum(id), (FromEnum(id) - FromEnum(Error::AssetInitializationBeforeEngineStartup)), hint);
		}
	}
}
