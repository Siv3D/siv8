...
namespace s3d
{
	namespace
	{
		...
		[[nodiscard]]
		double EvaluateBernstein(std::array<double, Count> values, const double t) noexcept
		{
			for (size_t remaining = Count; 1 < remaining; --remaining)
			{
				for (size_t i = 0; i < (remaining - 1); ++i)
				{
					values[i] = ((1.0 - t) * values[i] + t * values[i + 1]);
				}
			}
			return values[0];
		}
		...
		UnitPolynomialRoots<Count - 1> BernsteinRoots(const std::array<double, Count>& values) noexcept
		{
			...
			else
			{
				...
				const auto critical = BernsteinRoots(derivative);
		...
		ClosestPairCandidate ClosestLineBezier(const Line& line, const Bezier& curve) noexcept
		{
			...
			const auto stationary = BernsteinRoots(normalDerivative);
			...
			if ((minHeight < 0.0) && (0.0 < maxHeight))
			{
				...
				const auto crossings = BernsteinRoots(heights);
				for (size_t i = 0; i < crossings.count; ++i)
				{
					Consider(crossings.values[i]);
				}
			}
			// (B(t) - endpoint).B'(t) has degree 3 or 5. Its Bernstein
			// coefficients follow from multiplying the position and derivative bases.
			...
			for (const Vec2& endpoint : { line.start, line.end })
			{
				...
				for (size_t i = 0; i <= Degree; ++i)
				{
					...
					{
						coefficients[i + j] += (weights[i][j] * (controls[i] - endpoint).dot(controls[j + 1] - controls[j]));
					}
				}
				const auto roots = BernsteinRoots(coefficients);
