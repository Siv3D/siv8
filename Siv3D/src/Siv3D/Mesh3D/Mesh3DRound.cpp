...
namespace s3d::Mesh3DDetail
{
	namespace
	{
		using RoundCircleSample = Mesh3DDetail::CircleSample<float>;
	}
	...
	{
		return AppendAnnulus(mesh, 0.0, radius, segments);
	}
		...
		const uint32 segments)
	{
		...
		{
			return OperationFailed(Mesh3DErrorCode::NumericRange, U"Mesh3D::Annulus()/Disc(): The radii must be finite and float-representable");
		}
		...
		if ((innerRadius < 0.0f)
			...
			|| (segments < 3))
		{
			return OperationFailed(Mesh3DErrorCode::InvalidArgument, U"Mesh3D::Annulus()/Disc(): The radii or segment count is invalid");
		}
		...
		if (isDisc)
		{
			...
			{
				return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Disc(): The generated vertex count exceeds the supported range");
			}
		...
		else
		{
			...
			{
				return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Annulus(): The generated mesh exceeds the supported size");
			}
		}

		if (Mesh3D::MaxVertexCount < vertexCount)
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Annulus()/Disc(): The generated vertex count exceeds the supported range");
		}
		...
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Annulus()/Disc(): The generated mesh exceeds the supported size");
		}

		const Array<RoundCircleSample> circle = Mesh3DDetail::MakeCircleSamples<float>(segments);
		const Float3 normal = Float3::UnitY();
		const Float4 tangent{ 1.0f, 0.0f, 0.0f, 1.0f };

		if (isDisc)
		{
			mesh.vertices[vertexOffset] = Vertex3D{
				.pos = Float3::Zero(),
				.normal = normal,
			...
			{
				const RoundCircleSample sample = circle[i];
				mesh.vertices[vertexOffset + static_cast<size_t>(i) + 1] = Vertex3D{
				...
				mesh.indices[triangleOffset + i] = TriangleIndex32{ center, next, current };
			}
		}
		else
		{
			...
			{
				const RoundCircleSample sample = circle[i];
				mesh.vertices[outerRingBase + i] = Vertex3D{
					.pos = Float3{ (outerRadius * sample.cos), 0.0f, (outerRadius * sample.sin) },
					.normal = normal,
					.tex = Float2{ (0.5f + (0.5f * sample.cos)), (0.5f - (0.5f * sample.sin)) },
					.tangent = tangent
		...
		const uint32 segments)
	{
		...
		{
			return OperationFailed(Mesh3DErrorCode::NumericRange, U"Mesh3D::HollowCylinder(): The radii and height must be finite and float-representable");
		}
		...
		if ((innerRadius <= 0.0f)
			...
			|| (segments < 3))
		{
			return OperationFailed(Mesh3DErrorCode::InvalidArgument, U"Mesh3D::HollowCylinder(): The radii, height, or segment count is invalid");
		}
		...
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::HollowCylinder(): The generated mesh exceeds the supported size");
		}
		...
		for (uint32 i = 0; i <= segments; ++i)
		{
			...
			const RoundCircleSample outerSample = circle[i];
			...
			const RoundCircleSample innerSample = circle[segments - i];
		...
		const uint32 segments)
	{
		if ((not IsFloatRepresentable(_bottomRadius))
			...
			|| (not IsFloatRepresentable(_height)))
		{
			return OperationFailed(Mesh3DErrorCode::NumericRange, U"Mesh3D::ConicalFrustum()/Cylinder()/Cone(): The radii and height must be finite and float-representable");
		}
		...
		if ((bottomRadius <= 0.0f)
			...
			|| (segments < 3))
		{
			return OperationFailed(Mesh3DErrorCode::InvalidArgument, U"Mesh3D::ConicalFrustum()/Cylinder()/Cone(): The radii, height, or segment count is invalid");
		}
		...
		if (not CheckedAdd(static_cast<size_t>(segments), 1, ringStride))
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::ConicalFrustum()/Cylinder()/Cone(): segments exceed the supported range");
		}
		...
		if (isCone)
		{
			...
			if ((not CheckedMultiply(static_cast<size_t>(segments), 3, scaledSegments))
				...
				|| (not CheckedMultiply(static_cast<size_t>(segments), 2, triangleCount)))
			{
				return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Cone(): The generated mesh exceeds the supported size");
			}
		}
		else
		{
			...
			{
				return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::ConicalFrustum()/Cylinder(): The generated mesh exceeds the supported size");
			}
		}

		if (Mesh3D::MaxVertexCount < vertexCount)
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::ConicalFrustum()/Cylinder()/Cone(): The generated vertex count exceeds the supported range");
		}
		...
		if (not ResizeForAddition(
			mesh, vertexCount, triangleCount, vertexOffset, triangleOffset))
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::ConicalFrustum()/Cylinder()/Cone(): The generated mesh exceeds the supported size");
		}
		...
		if (isCone)
		{
			...
			for (uint32 i = 0; i < segments; ++i)
			{
				...
				mesh.vertices[apexBase + i] = Vertex3D{
	...
	Mesh3DAddResult AppendCylinder(
		...
		const uint32 segments)
	{
		return AppendConicalFrustum(mesh, radius, radius, height, segments);
	}
	...
	Mesh3DAddResult AppendCone(
		...
		const uint32 segments)
	{
		return AppendConicalFrustum(mesh, radius, 0.0, height, segments);
	}
		...
		const uint32 tubeSegments)
	{
		...
		{
			return OperationFailed(Mesh3DErrorCode::NumericRange, U"Mesh3D::Torus(): majorRadius and tubeRadius must be finite and float-representable");
		}
		...
		if ((majorRadius <= 0.0f)
			...
			|| (tubeSegments < 3))
		{
			return OperationFailed(Mesh3DErrorCode::InvalidArgument, U"Mesh3D::Torus(): The radii or segment counts are invalid");
		}
		...
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Torus(): The generated mesh exceeds the supported size");
		}
		...
		const Array<RoundCircleSample> ringSinCos = Mesh3DDetail::MakeCircleSamples<float>(ringSegments);
		const Array<RoundCircleSample> tubeSinCos = Mesh3DDetail::MakeCircleSamples<float>(tubeSegments);
		...
		const uint32 stacks)
	{
		...
		{
			return OperationFailed(Mesh3DErrorCode::NumericRange, U"Mesh3D::Sphere(): radius must be finite and float-representable");
		}
		...
		if ((radius <= 0.0f)
			...
			|| (stacks < 2))
		{
			return OperationFailed(Mesh3DErrorCode::InvalidArgument, U"Mesh3D::Sphere(): The radius, slice count, or stack count is invalid");
		}
		...
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Sphere(): The generated mesh exceeds the supported size");
		}
		...
		const Array<RoundCircleSample> longitudeSinCos = Mesh3DDetail::MakeCircleSamples<float>(slices);
		...
		for (uint32 x = 0; x < slices; ++x)
		{
			...
			mesh.vertices[vertexOffset + x] = Vertex3D{
	...
	Mesh3DAddResult AppendHemisphere(
		...
		const uint32 stacks)
	{
		return AppendHemisphere(mesh, radius, CloseBottom::No, slices, stacks);
	}
		...
		const uint32 stacks)
	{
		if (not IsFloatRepresentable(_radius))
		{
			return OperationFailed(Mesh3DErrorCode::NumericRange, U"Mesh3D::Hemisphere(): radius must be finite and float-representable");
		}
		...
		if ((radius <= 0.0f)
			...
			|| (stacks < 1))
		{
			return OperationFailed(Mesh3DErrorCode::InvalidArgument, U"Mesh3D::Hemisphere(): The radius, slice count, or stack count is invalid");
		}
		...
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Hemisphere(): The surface dimensions exceed the supported range");
		}

		triangleFactor = (twiceStackCount - 1);
		if (not CheckedMultiply(static_cast<size_t>(slices), triangleFactor, surfaceTriangleCount))
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Hemisphere(): The surface triangle count exceeds the supported range");
		}
		...
		if (closeBottom)
		{
			...
			{
				return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Hemisphere(): The closed-bottom mesh exceeds the supported size");
			}
		}

		if (Mesh3D::MaxVertexCount < vertexCount)
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Hemisphere(): The generated vertex count exceeds the supported range");
		}
		...
		if (not ResizeForAddition(
			mesh, vertexCount, triangleCount, vertexOffset, triangleOffset))
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Hemisphere(): The generated mesh exceeds the supported size");
		}
		...
		const uint32 hemisphereStacks)
	{
		...
		{
			return OperationFailed(Mesh3DErrorCode::NumericRange, U"Mesh3D::Capsule(): radius and cylinderHeight must be finite and float-representable");
		}
		...
		if ((radius <= 0.0f)
			...
			|| (hemisphereStacks < 1))
		{
			return OperationFailed(Mesh3DErrorCode::InvalidArgument, U"Mesh3D::Capsule(): The radius, cylinder height, slice count, or hemisphere stack count is invalid");
		}
		...
		if (cylinderHeight == 0.0f)
		{
			...
			{
				return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Capsule(): hemisphereStacks exceed the supported range for a zero-length cylinder");
			}

			return AppendSphere(mesh, radius, slices, (hemisphereStacks * 2));
		}
		...
		if ((not CheckedMultiply(static_cast<size_t>(hemisphereStacks), 2, interiorRingCount))
			...
			|| (not CheckedMultiply(triangleCount, 2, triangleCount)))
		{
			return OperationFailed(Mesh3DErrorCode::SizeLimit, U"Mesh3D::Capsule(): The generated mesh exceeds the supported size");
		}
